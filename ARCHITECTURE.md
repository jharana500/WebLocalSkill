...
`crypto.randomBytes(32)` generates the raw token; only its SHA-256 hash is stored (`PasswordReset.tokenHash`), with a 30-minute expiry and single-use enforcement (`usedAt`). `POST /api/auth/forgot-password` always returns the same neutral `200` message regardless of whether the email is registered, to avoid leaking account existence. No email service is configured, so in development the reset URL is logged server-side instead of emailed — this is gated behind `NODE_ENV !== 'production'` and never appears in the API response.
...
This project currently targets local/single-instance deployment: local file storage for uploads, no email service, no background job scheduler, no caching layer beyond TanStack Query's client-side cache. Deploying beyond local development would require, at minimum: swapping local file storage for an object store, wiring a real email provider for password resets, rotating `JWT_SECRET` and database credentials out of any example/dev values, and running Prisma migrations via `migrate deploy` in CI/CD rather than manually.
